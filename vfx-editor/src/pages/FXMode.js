    ...
    if (frames.length > 0 && currentFrameIndex < frames.length) {
      ...
      img.onload = () => {
        ...
        if (canvas) {
          ...
          // Draw masks for each object - using SAME approach as SegmentationMode
            ...
            objects.forEach((object, index) => {
              if (object.visible && object.masks && object.masks[currentFrameIndex]) {
                ...
                maskImg.onload = () => {
                  ...
                  const coloredMaskData = tempCtx.createImageData(canvas.width, canvas.height);
                  ...
                  // Apply the color to the mask with opacity from global settings
                  const opacity = globalParams.maskOpacity / 100;
                  
                  for (let i = 0; i < maskData.data.length; i += 4) {
                    // Check ALL channels (not just the first one)
                    if (maskData.data[i] > 0 || maskData.data[i+1] > 0 || 
                      ...
                      coloredMaskData.data[i + 3] = Math.round(255 * opacity); // Alpha
                    }
                  }
                    ...
                    Object.entries(object.effects).forEach(([effectType, effectFrames]) => {
                      if (effectFrames && effectFrames[currentFrameIndex]) {
                        ...
                        effectImg.onload = () => {
                          ...
                          // Set opacity from global settings
                          ctx.globalAlpha = globalParams.fxOpacity / 100;
                          
                          if (globalParams.invertFX) {
                            // Apply effect everywhere EXCEPT where the mask is
                            ctx.globalCompositeOperation = 'source-over';
                            ctx.drawImage(effectImg, 0, 0);
                            ctx.globalCompositeOperation = 'destination-out';
                            ctx.drawImage(maskImg, 0, 0);
                          } else {
                            // Apply effect ONLY where the mask is
                        ...
                        effectImg.onerror = (error) => {
                          console.error('Failed to load effect image:', fullEffectUrl, error);
                        };
                      }
                    });
                  }
                ...
                maskImg.onerror = (error) => {
                  console.error('Failed to load mask image:', fullMaskUrl, error);
                  ...
                  altMaskImg.onload = () => {
                    ...
                    for (let i = 0; i < maskData.data.length; i += 4) {
                      if (maskData.data[i] > 0 || maskData.data[i+1] > 0 || 
                        ...
                        coloredMaskData.data[i + 3] = Math.round(255 * opacity);
                      }
                    }
      ...
      img.onerror = (error) => {
        console.error('Failed to load frame image:', fullUrl, error);
        setError('Failed to load frame');
      };
    }
  }, [frames, currentFrameIndex, objects, globalParams, objectColors, videoId]);
